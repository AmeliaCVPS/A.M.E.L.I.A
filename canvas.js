  ...
  class NeuralBackground {
    ...
    createParticles() {
      ...
      const target = reducedMotion
        ? 24
        : this.lowPower
          ? clamp(Math.floor(area / 22000), 42, 84)
          : clamp(Math.floor(area / 13000), 72, 145);
      const neuralHues = [214, 196, 176, 154];
      this.particles = Array.from({ length: target }, () => {
        const speed = reducedMotion ? 0.016 : this.lowPower ? 0.056 : 0.074;
        return {
    ...
    draw(time, dt) {
      ...
      for (const p of this.particles) {
        ...
        p.x += p.vx * dt + Math.sin(time + p.phase) * 0.018 * dt;
        p.y += p.vy * dt + Math.cos(time * 0.8 + p.phase) * 0.016 * dt;
        p.vx *= 0.994;
        p.vy *= 0.994;
      ...
      const maxDist = this.lowPower ? 165 : 205;
      ...
      ctx.strokeStyle = this.lowPower ? "rgba(43,120,228,0.22)" : "rgba(43,120,228,0.32)";
      ctx.lineWidth = 1;
      ctx.stroke();
