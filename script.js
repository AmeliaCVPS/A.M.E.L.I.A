    ...
    if (inp) {
        inp.placeholder = q.hint
            || (q.type === "text" || q.type === "skip"
                ? "Digite sua resposta ou clique no 🎙️"
...
function _botMsgWithExtras(q) {
    ...
    typing.innerHTML = `<div class="av">🤖</div>
...
function _lockChoiceUI(wrap, selectedLabel) {
    if (selectedLabel) {
        wrap.innerHTML = `<div class="choice-selected">✔ ${_esc(selectedLabel)}</div>`;
    } else {
...
function _processAnswer(q, textAnswer, featureUpdates) {
    // Salva texto legível
    chatData.answers[q.id] = textAnswer;
...
function _extractFeaturesFromText(questionId, text) {
    ...
    if (questionId === "pain_level") {
        ...
        updates.pain_level = m ? parseInt(m[1]) :
            /insuportável|horrível|terrível|agonia/i.test(t) ? 10 :
            /muito forte|intensa|severa/i.test(t) ? 8 :
            /forte|considerável/i.test(t) ? 7 :
            /moderada|média|razoável/i.test(t) ? 5 :
            /leve|fraca|pouca/i.test(t) ? 3 :
            /sem dor|não doi|nenhuma/i.test(t) ? 0 : 5;
        ...
        updates.chest_pain           = /dor no peito|aperto.*peito|dor torácica|coração doendo|infarto/i.test(t);
        updates.vomiting             = /vômito|vomitando|náusea intensa|ânsia/i.test(t);
        updates.severe_headache      = /dor de cabeça|cefaleia|enxaqueca|cabeça latejando/i.test(t);
        updates.allergic_reaction    = /alergi|anafilaxia|urticária|coceira.*corpo|picada/i.test(t);
        updates.bleeding             = /sangue|sangrando|sangramento|hemorragia/i.test(t);
        updates.altered_consciousness= /desmaiei|convulsão|desorientado|confuso|perdi.*consciência/i.test(t);
        updates.trauma               = /acidente|caí|queda|bateu|trauma|colisão|machucou/i.test(t);
        ...
        else if (/hoje cedo|manhã/.test(t)) dur = 4;
...
function _classifyLocal() {
    ...
    const exps = {
        ...
        yellow: "🟡 Urgente — Fila prioritária.",
...
function _showResult(color, explanation, password, confidence, waitTime) {
    ...
    _botMsg(`
        ...
        ⏱️ Tempo de espera: <strong>${waitTime || "—"}</strong><br><br>
        🎫 Sua senha de atendimento:<br>
        <span style="font-family:var(--font-display);font-size:2.2rem;font-weight:900;
              color:var(--c-blue);letter-spacing:.1em">${password}</span><br><br>
        ${confLine}<br>
        Baixe o prontuário PDF e apresente no guichê.
    `);

    if (typeof speak === "function") {
        speak(`Triagem concluída. ${explanation.replace(/<[^>]*>/g,"")} ` +
...
function generatePDF() {
    ...
    doc.text("PRONTUÁRIO DE TRIAGEM", W/2, 16, {align:"center"});
    doc.setFontSize(11); doc.setFont(undefined,"normal");
    doc.text("A.M.E.L.I.A — Sistema de Triagem por Inteligência Artificial", W/2, 27, {align:"center"});

    // Dados do paciente
    let y = 48;
    doc.setFontSize(8); doc.setTextColor(100,116,139);
    doc.text("DADOS DO PACIENTE", M, y); y += 5;
    doc.setFontSize(10); doc.setTextColor(26,32,53); doc.setFont(undefined,"normal");
    doc.text(`Nome:       ${currentUser?.nome || "—"}`,       M, y); y += 6;
    doc.text(`CPF:        ${_maskCPF(currentUser?.cpf || "")}`, M, y);
    doc.text(`SUS:  ${_maskSUS(currentUser?.sus || "")}`, 110, y); y += 6;
    doc.text(`Data/Hora:  ${date} às ${time}`, M, y);

    y += 8; doc.setDrawColor(221,227,238); doc.line(M,y,W-M,y); y += 7;

    // Sintomas estruturados
    doc.setFontSize(8); doc.setTextColor(100,116,139); doc.text("SINTOMAS IDENTIFICADOS", M, y); y += 5;
    doc.setFontSize(10); doc.setTextColor(26,32,53);

    const f = chatData.features;
    const symptomRows = [
        [`Nível de dor: ${f.pain_level ?? "—"}/10`, `Duração: ${f.duration_hours ?? "—"}h`],
    ...
    // Relatos textuais
    doc.setFontSize(8); doc.setTextColor(100,116,139); doc.text("RELATO DO PACIENTE", M, y); y += 5;
    QUESTIONS.forEach((q, i) => {
        const ans = chatData.answers[q.id];
        if (!ans || q.id === "greeting") return;
        const lines = doc.splitTextToSize(`${q.text.replace(/<[^>]*>/g,"")} → ${ans}`, W-M*2);
        doc.setFontSize(9); doc.setTextColor(26,32,53); doc.setFont(undefined,"normal");
    ...
    const classMap = { U:"URGENTE / EMERGÊNCIA", M:"MÉDIA PRIORIDADE", L:"BAIXA PRIORIDADE" };
    doc.setFontSize(11); doc.setFont(undefined,"bold"); doc.setTextColor(26,32,53);
    doc.text(`CLASSIFICAÇÃO: ${classMap[chatData.classification] || "—"}`, M, y); y += 8;
    ...
    doc.text("Apresente no guichê · A.M.E.L.I.A v3.0 · FeNaDANTE 2025", W/2, 285, {align:"center"});
...
function _localPwd(color) {
    const p = color==="red"||color==="orange" ? "U" : color==="yellow" ? "M" : "L";
    _pwdCnt[p]++;
